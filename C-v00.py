...
dir_plots = "all-cont-" + scheme + "-plots" + \
    rf'-v{version:02d}'  # Place all plots in one place
dir_plots
...
def draw_samples(seed, 
                 loc=loc, cov=cov, d=d, 
                anchor_at=torch.linspace(start=0, end=1, steps=4),
                n_arm=n_arm,
                R=R, # rotation matrix
                ...
                effect_type=effect_type,
                 noise="indpt"): 
    """Draws samples for Experiment 3. 
    ...
    anchor_at : torch.Tensor
        Treatment levels with anchoring groups, 
        by default torch.linspace(start=0, end=1, steps=4).
    n_arm : int
        Size of each arm.
    R : function
    ...
    cov_trt : function
        The covariance matrix of the outcome distribution at time = 0 as a function of dose. 
    effect_type : str
    ...
    dict
        A dictionary with the following keys:
            - "lst_dt_all" : A concatenation of `lst_dt0` and `lst_dt1` (described below). 
            - "lst_dt0": A list of of tensors at time 0. 
            Each tensor includes the outcomes and the conditioning variables. 
            The last element corresponds to the "middle group" 
            while the remaining elements correspond to the "anchoring groups". 
            - "lst_dt1": A list of of tensors at time 0. 
            Each tensor includes the outcomes and the conditioning variables. 
            The last element corresponds to the "middle group" 
            while the remaining elements correspond to the "anchoring groups".         
    ...
    # Forces the use to supply a seed
    torch.manual_seed(seed)    
        
    # Samples for the anchoring groups 
    def get_anchor_sample(i, loc=loc, anchor_at=anchor_at, n_arm=n_arm): 
        tau = anchor_at[i]
        mvt = torch.distributions.MultivariateNormal(
            loc=loc + tau, 
            covariance_matrix=R(tau) @ cov_trt(tau) @ R(tau).t())
        # print(i)
        return torch.cat(
            # outcome variable
            [mvt.sample((n_arm, )),
             # conditioning variable
             torch.tensor([tau, 0]).unsqueeze(-2).expand(n_arm, -1)],
            dim=-1
        )
    
    # All observations from the anchoring group at t=0
    lst_dt_anchor0 = [get_anchor_sample(i) for i in range(len(anchor_at))]  

    # All observations from the anchoring group at t = 1
    lst_dt_anchor0_tmp = lst_dt_anchor0 if noise == "fixed" else [
        get_anchor_sample(i) for i in range(len(anchor_at))]
    
    def apply_trend(tmp): 
        # Must have all columns including the conditioning variables
        tau = tmp[:, -d_cond_var:][0, 0]  # treatment received
        return torch.cat(
            [p().transform(tmp[:, :d]),
             torch.tensor([tau, 1]).unsqueeze(-2).expand(tmp.shape[0], -1)],
            dim=-1
        )        
    
    # True counterfactual of anchoring groups
    lst_dt_anchor_count = [apply_trend(tmp=df) for df in lst_dt_anchor0_tmp]
    def get_observed(dt, tau): 
        dt_counterfactual = dt[:, :d]
        dt_cond_var = dt[:, -d_cond_var:]
        dt_observed = dt_counterfactual\
              + (tau if effect_type == "id" else torch.sqrt(tau))\
            .unsqueeze(-1)
        return torch.cat([dt_observed, dt_cond_var], dim=-1)
        
    # All observed data in anchoring groups
    lst_dt_anchor1 = [get_observed(dt, tau) for dt, tau in 
                      zip(lst_dt_anchor_count, anchor_at)]
    
    # Treatment levels for the (non-anchoring middle group)
    t = torch.rand(n_arm).clamp_min(1e-6).clamp_max(1 - 1e-6)
    
    # Create new mean vectors via broadcasting
    loc_new = t.unsqueeze(-1) + loc 

    # Outcomes for all subjects in the nonachoring middle group (i.e., 0 < dose < 1)
    def get_middle_outcome(t): 
        dt = torch.empty([n_arm, d])
    ...
    dt_middle0_outcome = get_middle_outcome(t) # at time = 0
    tmp02 = dt_middle0_outcome if noise == "fixed" else get_middle_outcome(t)    
    
    true_counterfactual_middle = p().transform(tmp02)
    dt_middle1_outcome = true_counterfactual_middle\
        + (t if effect_type == "id" else torch.sqrt(t))\
            .unsqueeze(-1)
    
    # Observed data for all treated patients in the middle, with conditioning variables
    dt_middle0 = torch.cat(
        [dt_middle0_outcome, 
         t.unsqueeze(-1), # tau
         torch.zeros([n_arm, 1])], # time point
         dim=-1
    )
    dt_middle1 = torch.cat(
        [dt_middle1_outcome, 
         t.unsqueeze(-1), # tau 
         torch.ones([n_arm, 1])], # time point
         dim=-1
    ) 

    # all observations at times 0 and 1
    lst_dt0 = [dt.detach() for dt in lst_dt_anchor0 + [dt_middle0]]   
    lst_dt1 = [dt.detach() for dt in lst_dt_anchor1 + [dt_middle1]] 
    lst_dt_all = lst_dt0 + lst_dt1

    return {
        'lst_dt_all': lst_dt_all, 
        'lst_dt0': lst_dt0, 
        'lst_dt1': lst_dt1
    }
...
def prepare_train_test_data(lst_dt, prop_validate=0.2, 
    ...
    lst_train_orig = [dt[:-int(dt.shape[0] * prop_validate), :]
                      for dt in lst_dt]
...
dict_sample = draw_samples(seed=0, noise="indpt")
lst_dt = dict_sample['lst_dt_all']
...
n_grps = int(len(lst_dt) / 2)
tmp = list(dict_dt['lst_train_std'][i] for i in [0, n_grps])
# len(tmp)
# tmp[0]
# tmp[1]

# Controls at times 0 and 1
lst_plot_std = list(dict_dt['lst_train_std'][i] for i in [0, n_grps])\
    + [ # Treated at time 0 (anchoring groups + middle group)
        torch.cat(
            list(dict_dt['lst_train_std'][i] for i in range(1, n_grps)), 
            dim=-2
        )
    ] \
        + [# Treated at time 1 (anchoring groups and middle group)
            torch.cat(
                list(dict_dt['lst_train_std'][i] for i in range(1 + n_grps, 2 *n_grps)),
                dim=-2
            )
        ]
...
titles_C = [r"$s_{(0, 0)}$", r"$s_{(0, 1)}$",
          r"$s_{(\tau, 0)}$", r"$s_{(\tau, 1)}$"]

for ax, dt, title in zip(axes.flat, lst_plot_std, titles_C):
...
flows = zuko.flows.NSF(features=d, context=d_cond_var,
                       transforms=5, bins=8, hidden_features=(128, 128,))
...
lst_dt0 = dict_sample['lst_dt0']
lst_dt1 = dict_sample['lst_dt1']

def estimate_mean_effects(lst_dt0=lst_dt0, 
                          lst_dt1=lst_dt1, 
                          n_arm=n_arm, 
                          d=d, device=device, 
                         dir_saved_vars=dir_saved_vars, 
    ...
    lst_dt0 : list
        ...
        This is expected to be extracted from a data dictionary of `draw_samples()`
        using the `'lst_dt0'` key. 
    lst_dt1 : list
        A complete list of observed outcome tensors on the *original* scale. 
        This is expected to be extracted from a data dictionary of `draw_samples()`
        using the `'lst_dt1'` key. 
    n_trted : int
    ...
    if fast and all(file in saved_files for file in filenames_ext):
        ...
        n_dose_mid = n_arm
        mean_effects = torch.empty([n_dose_mid, d], device=device)
        quantile_lower = torch.empty([n_dose_mid, d], device=device)
        quantile_upper = torch.empty([n_dose_mid, d], device=device)       


        # Sorted data
        lst_dt0_sorted = [sort_by_treatment(dt).to(device) for dt in lst_dt0]
        lst_dt1_sorted = [sort_by_treatment(dt).to(device) for dt in lst_dt1]    
        
        sorted_mid_0 = lst_dt0_sorted[-1]
        sorted_mid_1 = lst_dt1_sorted[-1]

        dt_to_be_sent0 = torch.cat(lst_dt0_sorted, dim=-2)
        dt_to_be_sent1 = torch.cat(lst_dt1_sorted, dim=-2)

        def move_data(dt_to_be_sent, df_mid, i):            
            tmp = cocycle(c_to=df_mid[i, -d_cond_var:],
                          c_from=dt_to_be_sent[:, -d_cond_var:],
                          y=dt_to_be_sent[:, :d],
                          flows=flows)
            return tmp
        
        for i in range(n_dose_mid):
            with torch.no_grad():
                ...
                est_observed = move_data(dt_to_be_sent=dt_to_be_sent1, 
                                         df_mid=sorted_mid_1, 
                                         i=i)
...
# torch.allclose(dt_to_be_sent0[:, -d_cond_var:][:, 0], 
#                dt_to_be_sent1[:, -d_cond_var:][:, 0])

t = sorted_mid_0[:, -d_cond_var:][:, 0]
